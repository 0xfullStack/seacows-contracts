...
describe("MockSeacowsComplement", () => {
  ...
  it("Should get the complemented balance correctly", async () => {
    const [tokenBalance, nftBalance] = await MockSeacowsComplement.getComplementedBalance(MockERC20.address, MockERC721.address);
  ...
  it("should output more tokens when less NFTs are output", async () => {
    ...
    await (await MockSeacowsComplement.updateComplement(ethers.utils.parseEther("0.2"), ethers.utils.parseEther("0.2"))).wait();
